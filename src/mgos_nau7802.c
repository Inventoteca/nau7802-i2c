...
static void mgos_nau7802_irq(int pin, void *arg) {
...
struct mgos_nau7802 *mgos_nau7802_create(struct mgos_i2c *i2c, int drdy_gpio) {
  ...
  dev->i2caddr = NAU7802_I2CADDR_DEFAULT;
  dev->i2c = i2c;
  dev->drdy_gpio = drdy_gpio;

  int revision_id;
  if ((revision_id = mgos_i2c_read_reg_b(dev->i2c, dev->i2caddr,
                                         MGOS_NAU7802_REVISION_ID)) != 0x0F) {
    LOG(LL_ERROR,
        ("Error reading revision ID register: expected 0x0F, got=0x%02x",
         revision_id));
    goto create_failed;
  }

  if (dev->drdy_gpio != -1) {
    LOG(LL_INFO, ("Installing interrupt handler on GPIO %d", dev->drdy_gpio));
    mgos_gpio_set_mode(dev->drdy_gpio, MGOS_GPIO_MODE_INPUT);
    mgos_gpio_set_pull(dev->drdy_gpio, MGOS_GPIO_PULL_UP);
    mgos_gpio_set_int_handler(dev->drdy_gpio, MGOS_GPIO_INT_EDGE_NEG,
                              mgos_nau7802_irq, dev);
    mgos_gpio_clear_int(dev->drdy_gpio);
    mgos_gpio_enable_int(dev->drdy_gpio);
  }
  LOG(LL_INFO,
      ("NAU7802 initialized %sat I2C 0x%02x",
       (dev->drdy_gpio != -1) ? "with interrupts " : " ", dev->i2caddr));
  return dev;
create_failed:
  free(dev);
  return NULL;
...
bool mgos_nau7802_destroy(struct mgos_nau7802 **dev) {
  ...
  if ((*dev)->drdy_gpio != -1) {
    LOG(LL_INFO, ("Removing interrupt handler on GPIO %d", (*dev)->drdy_gpio));
...
bool mgos_nau7802_init(struct mgos_nau7802 *dev) {
  ...
  mgos_i2c_write_reg_b(dev->i2c, dev->i2caddr, MGOS_NAU7802_PU_CTRL,
...
bool mgos_nau7802_reset(struct mgos_nau7802 *dev) {
  ...
  mgos_i2c_write_reg_b(dev->i2c, dev->i2caddr, MGOS_NAU7802_PU_CTRL,
                       pu_ctlr | MGOS_NAU7802_PU_CTRL_RR);
  mgos_usleep(1);
  mgos_i2c_write_reg_b(dev->i2c, dev->i2caddr, MGOS_NAU7802_PU_CTRL,
                       pu_ctlr & ~MGOS_NAU7802_PU_CTRL_RR);
...
bool mgos_nau7802_set_ldo(struct mgos_nau7802 *dev, uint8_t ldo_value) {
  ...
  mgos_i2c_write_reg_b(
    ...
    ctlr1 | ((ldo_value << MGOS_NAU7802_CTRL1_VLDO_SHIFT) & MGOS_NAU7802_CTRL1_VLDO_MASK)
  );
...
bool mgos_nau7802_set_gain(struct mgos_nau7802 *dev, uint8_t gain_value) {
  ...
  mgos_i2c_write_reg_b(
    dev->i2c, dev->i2caddr, MGOS_NAU7802_CTRL1,
    ctlr1 | ((gain_value << MGOS_NAU7802_CTRL1_GAIN_SHIFT) & MGOS_NAU7802_CTRL1_GAIN_MASK)
  );
...
bool mgos_nau7802_set_sps(struct mgos_nau7802 *dev, uint8_t sps_value) {
  ...
  mgos_i2c_write_reg_b(
    ...
    ctlr2 | ((sps_value << MGOS_NAU7802_CTRL2_CRS_SHIFT) & MGOS_NAU7802_CTRL2_CRS_MASK)
  );
...
bool mgos_nau7802_calibrate(struct mgos_nau7802 *dev) {
  ...
  if (ctlr2 & MGOS_NAU7802_CTRL2_CAL_ERROR) {
    LOG(LL_ERROR, ("Error calibrating up NAU7802: CTRL2_CAL_ERROR"));
    return false;
  }
...
int32_t mgos_nau7802_read_data(struct mgos_nau7802 *dev) {
  if (!dev) {
    return 0;
  }
